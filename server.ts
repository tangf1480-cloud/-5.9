...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data?: { skin?: string }) => {
    const name = `Snake-${snakeCounter++}`;
    const color = COLORS[Math.floor(Math.random() * COLORS.length)];
    const defaultSkins = ['🐱', '🐶', '🐭', '🐹', '🐰', '🦊', '🐻', '🐼', '🐨', '🐯', '🦁', '🐮', '🐷', '🐸', '🐵'];
    const skin = data?.skin || defaultSkins[Math.floor(Math.random() * defaultSkins.length)];
...
setInterval(() => {
  ...
  state.leaderboard = Object.values(state.players)
    ...
    .slice(0, 10)
    .map(p => ({ id: p.id, name: p.name, score: Math.floor(p.score), color: p.color, skin: p.skin }));
